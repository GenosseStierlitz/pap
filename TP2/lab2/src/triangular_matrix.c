...
#pragma omp parallel for schedule(static)  
  for ( i = 0 ; i < N ; i = i + 1)
    {
#pragma omp parallel default (none) shared(i, M, b, c) private (r, j)
 {        
      r = 0.0 ;
#pragma omp parallel for schedule(static)  
      for (j = 0 ; j <= i ; j = j + 1)
...
#pragma omp parallel for schedule(dynamic) shared(M, b, c) private (r, j, i)
  for ( i = 0 ; i < N ; i = i + 1)
    {
      r = 0.0 ;
      for (j = 0 ; j <= i ; j = j + 1)
...
#pragma omp parallel for schedule(guided, 16) shared(M, b, c) private (r, j, i)
  for ( i = 0 ; i < N ; i = i + 1)
    {
      r = 0.0 ;
      for (j = 0 ; j <= i ; j = j + 1)
 ...
 */
  ...
  register double r;
  {
#pragma omp parallel for shared(M, b, c) private (r, j, i)
  for ( i = 0 ; i < N ; i = i + 1)
    {
      r = 0.0 ;
      for (j = 0 ; j <= i ; j = j + 1)
